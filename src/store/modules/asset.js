...
const actions= {
    ...
    * @param {
    ...
    /**
     * V2.0 获取 进线设备(设施)列表 ,台区+配电柜 列表
     * @param {Number} id 台区ID 
     * @param {Number} projectId 项目ID
     */
    getIncomingFacility({commit},obj){
        return request({
            method:'post',
            url:`${api.getIncomingFacility}`,
            data:{
                ...obj
            }
        }).then(res=>{
    ...
    /**
     * V2.0 获取 "主进线/备用进线" 列表
     * @param {Number} courtsId 台区ID 
     * @param {Number} branchesId 分支箱ID
     * @param {Number} chestId 配电柜ID
     * ⭐ 以上必填一个
     */
    getIncomingLine({commit},obj){
        return request({
            method:'post',
            url:`${api.getIncomingLine}`,
            data:{
                ...obj
            }
        }).then(res=>{
    ...
    * @param {
     ...
     * @param {
        ...
        *      current 页码
        *      projectId 项目id,0查询(查询主线缆列表不用传)
        *       filterStr":"11111,修改1,1位置,修改1"
        * }
        */
    getCableTrenchList({commit},obj){
        return request({
            method:'post',
            url:`${api.cableTrenchList}`,
            data:obj
        }).then(res=>{
            if(res && res.code === 10000000 && res.data){
                return res;
    ...
    * @param {
        ...
        * }
        */
    CreateCreateCableTrench({commit},obj){
        return request({
            method:'post',
            url:`${api.createCableTrench}`,
            data:obj
        }).then(res=>{
            if(res && res.code===10000000){
                tip(res.meassage,'success')
     ...
     * @param Integer id 电缆通道ID
     * @param Integer isDelete 是否删除 0否 1是
     * @param String location 位置
     * @param String name 名称
     * @param String number 编号
     * @param Integer projectId 项目ID
     */
    editAndDeleteCableTrench({commit},obj){
        return request({
            method:'post',
            url:`${api.editAndDeleteCableTrench}`,
            data:obj
        }).then(res=>{
            if(res && res.code===10000000){
                tip("编辑成功",'success')

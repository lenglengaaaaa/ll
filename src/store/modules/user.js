...
const actions= {
    ...
    login({ commit }, userInfo) {
        ...
        return request({
                method:'post',
                url:`${api.login_mini}?accountName=${username}&password=${password}`,
            }).then(res=>{
                if(res && res.code===10000000 && res.data){
                    const { jtoken, user_detail } = res.data;

                    //setUserDetail
                    let { projects } = user_detail;
                    let userDetail = {
                        ...user_detail,
    ...
    getAccountDetail({commit},id){
        return request({
            ...
            if(res&&res.code === 10000000&&res.data){
                if( !store.state.user.userDetail ) return; 
                const { projectIds, projects } = store.state.user.userDetail;
                let user_detail = {
                    ...res.data,
                    projectIds,
                    projects
